...
namespace AncientGod.System
{
    ...
    {
        public static int randomXLeft = WorldGen.genRand.Next(300, Main.maxTilesX / 4);//太空基地坐标初始值
        public static int randomXRight = WorldGen.genRand.Next(Main.maxTilesX * 3 / 4, Main.maxTilesX - 300);//太空基地坐标初始值

        // 生成一个随机数（0或1）
        public static int randomNumber = new Random().Next(2);

        int houseX = (randomNumber == 0) ? randomXLeft : randomXRight;//不能让太空基地离玩家出生点太近,只能在两边
        ...
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            //创建基础建筑结构
            LoadBearingColumn(houseX + 40, houseY + 42, 30, 45);
            LoadBearingColumn(houseX - 40, houseY + 42, 30, 45);

            DetectionTower(houseX + 60, houseY - 20, 10, 40);
            DetectionTower(houseX - 60, houseY - 20, 10, 40);
            DetectionTower(houseX + 90, houseY - 16, 15, 30);
            DetectionTower(houseX - 90, houseY - 16, 15, 30);
            DetectionTower(houseX + 120, houseY - 3, 10, 30);
            DetectionTower(houseX - 120, houseY - 3, 10, 30);

            SignalTower(houseX + 40, houseY - 30, 10, 40);
            SignalTower(houseX - 40, houseY - 30, 10, 40);
            SignalTower(houseX + 70, houseY - 18, 15, 45);
            SignalTower(houseX - 70, houseY - 18, 15, 45);
            SignalTower(houseX + 140, houseY - 1, 8, 30);
            SignalTower(houseX - 140, houseY - 1, 8, 30);

            SignalTower(houseX + 40, houseY + 80, 20, 50);
            SignalTower(houseX - 40, houseY + 80, 20, 50);
            SignalTower(houseX + 70, houseY + 74, 15, 40);
            SignalTower(houseX - 70, houseY + 74, 15, 40);
            SignalTower(houseX + 140, houseY + 65, 10, 30);
            SignalTower(houseX - 140, houseY + 65, 10, 30);

            ShelfTower(houseX + 100, houseY + 10, 50, 20);
            ShelfTower(houseX - 100, houseY + 10, 50, 20);
            ShelfTower(houseX + 110, houseY + 56, 40, 15);
            ShelfTower(houseX - 110, houseY + 56, 40, 15);
            ShelfTower(houseX + 130, houseY + 15, 45, 15);
            ShelfTower(houseX - 130, houseY + 15, 45, 15);
            ShelfTower(houseX + 150, houseY + 57, 40, 20);
            ShelfTower(houseX - 150, houseY + 57, 40, 20);
            ShelfTower(houseX + 30, houseY - 24, 40, 20);
            ShelfTower(houseX - 30, houseY - 24, 40, 20);
        ...
        private void SignalTower(int centerX, int centerY, int width, int height)//信号塔结构
        {
            for (int x = centerX - width / 2; x <= centerX + width / 2; x++)
            {
                for (int y = centerY - height / 2; y <= centerY + height / 2; y++)
                {
                    if (centerX - houseX > 0)//如果在右
                    {
                        if (centerY - houseY < 0)//如果在右上
                        {
