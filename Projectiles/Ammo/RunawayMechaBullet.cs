...
{
    public class RunawayMechaBullet : ModProjectile
    {
        ...
        public override void SetDefaults()
        {
            ...
            Projectile.friendly = false; // 投射物能对敌人造成伤害吗？
            Projectile.hostile = true; // 投射物能对玩家造成伤害吗？
            Projectile.DamageType = DamageClass.Ranged; // 投射物的伤害类型，是由远程武器发射的吗？
            Projectile.penetrate = 1; // 投射物能穿透多少个敌怪。（OnTileCollide方法也会减少穿透次数）
        ...
        private void Explode()//添加爆炸效果
        {
            ...
            {
                if (player.active && Vector2.Distance(player.Center, Projectile.Center) < explosionRadius)
                {
                    // 这里可以根据需要调整伤害值
                    int damage = 20;
                    // 在这里扣除玩家的生命值
                    player.Hurt(PlayerDeathReason.ByCustomReason(player.name + " 被失控机甲整自闭了,十分甚至九分地自闭 "), damage, -10);
                }
            }
        }
