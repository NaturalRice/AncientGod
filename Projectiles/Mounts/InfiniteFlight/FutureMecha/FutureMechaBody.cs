...
{
    internal class FutureMechaBody : ModProjectile
    {
        ...
        public override void AI()//未来机甲具有使敌人慢慢变静止的属性
        {
            ...
            if (!modPlayer.FutureMecha)
            {
                Projectile.timeLeft = 0;
                Projectile.active = false;
            }
            if (modPlayer.FutureMecha && Owner.mount.Active)//当然，还要再在这里强调一遍才能达到效果
            {
                        ...
                        if (fireCooldown <= 0f)
                        {
                            ...
                            if (Projectile.localAI[0] == 0f)
                            {
                                ...
                                for (int j = 0; j < 1; j++) // 你可以根据需要发射多个弹药（这里不是间隔的连射，而是一次性射多少弹药）
                                {
                                    ...
                                    int newProjectile11 = Projectile.NewProjectile(null, Projectile.position + Vector2.UnitX * 170 + Vector2.UnitY * 150, shotVelocity, ModContent.ProjectileType<FutureMechaBullet>(), (int)(Projectile.damage * 0.5f), 0, Main.myPlayer);//右下
                                    int newProjectile12 = Projectile.NewProjectile(null, Projectile.position + Vector2.UnitX * 170 + Vector2.UnitY * 150, Vector2.UnitX.RotatedBy(shootAngle - 50) * 8f, ModContent.ProjectileType<FutureMechaBulletSide>(), (int)(Projectile.damage * 0.5f), 0, Main.myPlayer);//右下
                                    int newProjectile13 = Projectile.NewProjectile(null, Projectile.position + Vector2.UnitX * 170 + Vector2.UnitY * 150, Vector2.UnitX.RotatedBy(shootAngle + 50) * 8f, ModContent.ProjectileType<FutureMechaBulletSide>(), (int)(Projectile.damage * 0.5f), 0, Main.myPlayer);//右下
                                    ...
                                    int newProjectile21 = Projectile.NewProjectile(null, Projectile.position - Vector2.UnitX * 140 + Vector2.UnitY * 50, shotVelocity, ModContent.ProjectileType<FutureMechaBullet>(), (int)(Projectile.damage * 0.5f), 0, Main.myPlayer);//左上
                                    int newProjectile22 = Projectile.NewProjectile(null, Projectile.position - Vector2.UnitX * 140 + Vector2.UnitY * 50, Vector2.UnitX.RotatedBy(shootAngle - 50) * 8f, ModContent.ProjectileType<FutureMechaBulletSide>(), (int)(Projectile.damage * 0.5f), 0, Main.myPlayer);//左上
                                    int newProjectile23 = Projectile.NewProjectile(null, Projectile.position - Vector2.UnitX * 140 + Vector2.UnitY * 50, Vector2.UnitX.RotatedBy(shootAngle + 50) * 8f, ModContent.ProjectileType<FutureMechaBulletSide>(), (int)(Projectile.damage * 0.5f), 0, Main.myPlayer);//左上
                                    direction = targetNPC.Center - (Projectile.position + Vector2.UnitX * 195 + Vector2.UnitY * 50);
                                    shootAngle = direction.ToRotation();
                                    shotVelocity = Vector2.UnitX.RotatedBy(shootAngle) * 8f;
                                    int newProjectile31 = Projectile.NewProjectile(null, Projectile.position + Vector2.UnitX * 195 + Vector2.UnitY * 50, shotVelocity, ModContent.ProjectileType<FutureMechaBullet>(), (int)(Projectile.damage * 0.5f), 0, Main.myPlayer);//右上
                                    int newProjectile32 = Projectile.NewProjectile(null, Projectile.position + Vector2.UnitX * 195 + Vector2.UnitY * 50, Vector2.UnitX.RotatedBy(shootAngle - 50) * 8f, ModContent.ProjectileType<FutureMechaBulletSide>(), (int)(Projectile.damage * 0.5f), 0, Main.myPlayer);//右上
                                    int newProjectile33 = Projectile.NewProjectile(null, Projectile.position + Vector2.UnitX * 195 + Vector2.UnitY * 50, Vector2.UnitX.RotatedBy(shootAngle + 50) * 8f, ModContent.ProjectileType<FutureMechaBulletSide>(), (int)(Projectile.damage * 0.5f), 0, Main.myPlayer);//右上
        ...
        public override bool PreDrawExtras()//用于在绘制前执行额外的逻辑。其中，DrawChain 方法用于绘制链条效果
        {
            ...
            Texture2D teslaTex = (ModContent.Request<Texture2D>("AncientGod/Projectiles/Mounts/InfiniteFlight/FutureMecha/FutureMechaTesla")).Value;
            Texture2D laserTex = (ModContent.Request<Texture2D>("AncientGod/Projectiles/Mounts/InfiniteFlight/FutureMecha/FutureMechaLaser")).Value;
            Texture2D nukeTex = (ModContent.Request<Texture2D>("AncientGod/Projectiles/Mounts/InfiniteFlight/FutureMecha/FutureMechaNuke")).Value;
            Texture2D plasmaTex = (ModContent.Request<Texture2D>("AncientGod/Projectiles/Mounts/InfiniteFlight/FutureMecha/FutureMechaPlasma")).Value;
        ...
        public void DrawSingleArm(Texture2D handTex, Vector2 handPosition, float rotation, Vector2 offset, bool top, bool infernum = false)
        {
            ...
            Texture2D armTex = ModContent.Request<Texture2D>("AncientGod/Projectiles/Mounts/InfiniteFlight/FutureMecha/FutureMechaArm").Value;//大臂
            Texture2D forearmTex = ModContent.Request<Texture2D>("AncientGod/Projectiles/Mounts/InfiniteFlight/FutureMecha/FutureMechaForearm").Value;//小臂
        ...
        {
            Texture2D chainTex = ModContent.Request<Texture2D>("AncientGod/Projectiles/Mounts/InfiniteFlight/FutureMecha/FutureMechaChain").Value;
        ...
        {
            Texture2D eyesTex = ModContent.Request<Texture2D>("AncientGod/Projectiles/Mounts/InfiniteFlight/FutureMecha/FutureMechaEyes").Value;
