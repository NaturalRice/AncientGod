...
{
    public class FourDimensionalInsect : ModMount
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            MountData.buff = ModContent.BuffType<FourDimensionalInsectBuff>();//坐骑提供的Buff类型
            MountData.spawnDust = 33;//使用粒子效果类型33
            MountData.spawnDustNoGravity = true;//粒子效果将不受重力影响，会漂浮在空中

            MountData.totalFrames = 1;//帧数(目前只有一帧~~~~
            ...
            MountData.flyingFrameCount = MountData.inAirFrameCount = MountData.idleFrameCount = MountData.swimFrameCount = 1;
            //设置了坐骑在不同状态下的帧数。坐骑通常具有不同的动画帧数，以便在不同情况下播放不同的动画
            MountData.flyingFrameDelay = MountData.inAirFrameDelay = MountData.idleFrameDelay = MountData.swimFrameDelay = 1;
        ...
        public override bool Draw(List<DrawData> playerDrawData, int drawType, Player drawPlayer, ref Texture2D texture, ref Texture2D glowTexture, ref Vector2 drawPosition, ref Rectangle frame, ref Color drawColor, ref Color glowColor, ref float rotation, ref SpriteEffects spriteEffects, ref Vector2 drawOrigin, ref float drawScale, float shadow)
        {
            ...
            Texture2D mountTex = ModContent.Request<Texture2D>("AncientGod/Items/Mounts/InfiniteFlight/FourDimensionalInsect/FourDimensionalInsect_Back_Glow").Value;
